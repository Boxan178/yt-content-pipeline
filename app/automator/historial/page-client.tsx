...
export default function HistorialClient() {
  ...
  return (
    <main className="min-h-full px-6 py-6">
      {/* Hero (sin header propio — layout root ya trae sidebar+topbar) */}
      <section className="mb-6">
        <div className="flex items-start gap-4">
          <div className="w-12 h-12 rounded-lg bg-amber-500/15 border border-amber-500/30 flex items-center justify-center">
            <FolderIcon className="w-6 h-6 text-amber-400" />
          </div>
          <div className="flex-1">
            <h1 className="text-2xl font-bold tracking-tight">Historial Automator</h1>
            <p className="text-sm text-zinc-400 mt-1">
              Explora, reabre y regenera tus proyectos de vídeo generados con KIE.
            </p>
            {data?.output_root && (
              <p className="text-[11px] text-zinc-500 mt-1 font-mono">{data.output_root}</p>
            )}
          </div>
          <Link
            href="/automator"
